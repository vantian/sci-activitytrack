...
{
    public abstract class ActivityTrackerWebTestBase : AbpAspNetCoreIntegratedTestBase<Startup>
    {
        protected static readonly Lazy<string> ContentRootFolder;

        static ActivityTrackerWebTestBase()
        {
        ...
        protected override IWebHostBuilder CreateWebHostBuilder()
        {
            return base
                ...
                .UseSetting(WebHostDefaults.ApplicationKey, typeof(ActivityTrackerWebMvcModule).Assembly.FullName);
        }
        ...
        {
            using (var context = IocManager.Resolve<ActivityTrackerDbContext>())
            {
                action(context);
                context.SaveChanges();
            }
        ...
        protected async Task UsingDbContextAsync(Func<ActivityTrackerDbContext, Task> action)
        {
            using (var context = IocManager.Resolve<ActivityTrackerDbContext>())
            {
        ...
        protected async Task<T> UsingDbContextAsync<T>(Func<ActivityTrackerDbContext, Task<T>> func)
        {
            T result;
